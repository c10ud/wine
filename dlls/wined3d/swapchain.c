        ...
        struct wined3d_context *context, const RECT *src_rect, const RECT *dst_rect)
{
    ...
    if (gl_info->fbo_ops.glBlitFramebuffer && is_identity_fixup(backbuffer->resource.format->color_fixup))
    {
        ...
        if (backbuffer->resource.multisample_type)
        {
            ...
            wined3d_texture_load_location(swapchain->back_buffers[0], 0, context, location);
        }
        ...
        const RECT *dst_rect_in, const RGNDATA *dirty_region, DWORD flags)
{
    ...
    {
        wined3d_texture_load_location(swapchain->back_buffers[0], 0, context, WINED3D_LOCATION_TEXTURE_RGB);
...
void x11_copy_to_screen(const struct wined3d_swapchain *swapchain, const RECT *rect)
{
    ...
    wined3d_texture_load_location(front, 0, NULL, WINED3D_LOCATION_DIB);
