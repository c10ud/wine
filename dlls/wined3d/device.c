        ...
        DWORD location, const RECT *draw_rect, UINT rect_count, const RECT *clear_rect, RECT *out_rect)
{
    ...
    if (ds->container->sub_resources[ds->sub_resource_idx].locations & WINED3D_LOCATION_DISCARDED)
    {
    ...
    if (ds->container->sub_resources[ds->sub_resource_idx].locations & location)
        ...
        if (rt && rt->resource.format->id != WINED3DFMT_NULL)
        {
            if (fb->render_targets[i])
            {
                ...
                if (flags & WINED3DCLEAR_TARGET && !is_full_clear(target, draw_rect, clear_rect))
                    wined3d_texture_load_location(rt, fb->render_targets[i]->sub_resource_idx,
                            context, rt->resource.draw_binding);
                else
    ...
    if (flags & WINED3DCLEAR_TARGET)
    {
        for (i = 0; i < rt_count; ++i)
        {
            if (fb->render_targets[i])
            {
                ...
                wined3d_texture_validate_location(rt, fb->render_targets[i]->sub_resource_idx,
                        rt->resource.draw_binding);
                wined3d_texture_invalidate_location(rt, fb->render_targets[i]->sub_resource_idx,
                        ~rt->resource.draw_binding);
            }
        }
